<!-- English version: value_engine.md -->
...
L'agent reçoit `raw.json` + `RUBRIC.fr.md` + `HOURLY_RATES.fr.md` + état projet (`docs/architecture.fr.md` + `docs/state.md` du round si présent).
...
- **taux_BTC_par_heure** : selon profil requis par la nature de l'output, table publique versionnée (`rounds/HOURLY_RATES.fr.md`).
...
Les taux du `HOURLY_RATES.fr.md` sont des **variables** liées au marché freelance, exprimées en BTC à un instant donné. Recalibration trimestrielle :
...
Détail dans `rounds/RUBRIC.fr.md`. Synthèse :
...
Phase 1 MVP en GitHub Actions + multisig Safe. Pas de smart contract custom encore. Voir `rounds/scripts/` pour le squelette de la GitHub Action de collecte, `rounds/agent/PROMPT.fr.md` pour le prompt système, et `contracts/README.fr.md` pour la roadmap des contracts à venir.

## 11. Voir aussi

- Architecture générale du projet → [`architecture.fr.md`](architecture.fr.md)
